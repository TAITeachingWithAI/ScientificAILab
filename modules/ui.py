...
def ai_note():
    """Caption: which model powers the chat, and that the AI can be incorrect."""
    try:
        provider, model = llm.current_model()
        name = model.split("/")[-1] if model else "an AI model"
        prefix = f"🧠 Powered by the free **{name}** model (via {provider.capitalize()}). "
    except Exception:  # noqa: BLE001 - never let the note break the page
        prefix = ""
    st.caption(prefix + AI_NOTE)
